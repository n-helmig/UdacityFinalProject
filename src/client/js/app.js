...
import { geonamesApi, openWeatherApi, weatherBitApi, pixabayApi, geonamesUrl, openWeatherUrl,weatherBitCurrent, weatherBitForecast, pixabayUrl } from "../../server/apiUrls";
...
const getDestinationInfo = async function(destination){
	try {
		const destinationLocation = await fetch(openWeatherUrl + destination + openWeatherApi);
...
//Gets the current weather at the destination
const getCurrentWeatherInfo = async function(destinationInfo){
	const weatherInfo = await fetch(weatherBitCurrent + destinationInfo.lat.toString() + "&lon=" + destinationInfo.lon.toString() + weatherBitApi);
...
//Gets the forecast for the destination
const getForecastWeatherInfo = async function(destinationInfo){
	const weatherInfo = await fetch(weatherBitForecast + destinationInfo.lat.toString() + "&lon=" + destinationInfo.lon.toString() + weatherBitApi);
...
//Takes all information from API's and creates the plan
const createPlan = async function(){	
    ...
    try {
		...
		const getImage = await fetch(pixabayUrl + pixabayApi + "&q=" + recentDestination.city +"&image_type=photo&orientation=horizontal&category=travel&min_width=620&min_height=420");
...
export { weatherAtDestination, getDestinationInfo, getCurrentWeatherInfo, getForecastWeatherInfo, postWeatherInfo, createPlan, moveEntry}